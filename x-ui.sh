...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/hossinasaadi/x-ui/multi-user-inbound-dev/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/hossinasaadi/x-ui/raw/multi-user-inbound-dev/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
